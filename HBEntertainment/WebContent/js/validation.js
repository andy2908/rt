...
function validateFormArray(selector){
	for(var i=0;i<selector.length;i++){
		...
		if(value=="" && isVisible){
			alert($(selector[i]).attr("title") + " cannot be empty!!");
...
function validateForm(element){
	...
	for(var i=0;i<selector.length;i++){
		...
		if(tagName == "input" || tagName == "INPUT"){
			var value = $(selector[i]).val();
			if(value==""){
				alert($(selector[i-1]).html() + " cannot be empty!!");
				setTimeout(function(){$(selector[i]).focus()},1);
				return false;
			}
		}
...
//			console.log("cnt:::"+cnt);
	...
	$('.validateMobile').bind('keypress', function (event) {
	    ...
	    if (!regex.test(key) && s != "8" && s != "9"  && s!="37" && s != "38" && s != "39" && s!="40") {
	    	console.log("keycode:::"+s);
	       event.preventDefault();
	       return false;
	    }else if(s == "8" || s == "9" || s=="37" || s=="38" || s=="39" || s=="40"){
	    	console.log("keycode:::"+s);
	    	return true;
	    }
	...
	$('.validateAadharEnrol').bind('blur', function (event) {
		...
		if(value.length<28 && value.length>0){
			alert("Please Enter Valid Aadhar Enrollment No.!!");
			setTimeout(function(){$("#"+id).val("").focus();},1);
			return false;
		}else{
			return true;
	...
	$(".minorValidation").bind('blur',function(event){
		...
		if(!regex.test(value)&&value!=""){
			...
			if(age<18){
				alert("Insurer Age Must Be 18 Or 18+");
				return false;
			}else{
				return true;
			}
